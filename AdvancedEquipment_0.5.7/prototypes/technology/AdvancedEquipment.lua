data:extend(
{
  
  {
    type = "technology",
    name = "energy-shield-mk3-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/energy-shield-mk3-equipment.png",
    prerequisites = {"energy-shield-mk2-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "energy-shield-mk3-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 45
    },
    order = "g-e-b"
  },
  {
    type = "technology",
    name = "energy-shield-mk4-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/energy-shield-mk4-equipment.png",
    prerequisites = {"energy-shield-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "energy-shield-mk4-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 3}, {"science-pack-2", 3}, {"science-pack-3", 3}},
      time = 60
    },
    order = "g-e-b"
  },
  {
    type = "technology",
    name = "energy-shield-mk5-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/energy-shield-mk5-equipment.png",
    prerequisites = {"energy-shield-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "energy-shield-mk5-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}},
      time = 70
    },
    order = "g-e-b"
  },
  {
    type = "technology",
    name = "energy-shield-mk6-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/energy-shield-mk6-equipment.png",
    prerequisites = {"energy-shield-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "energy-shield-mk6-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 5}, {"science-pack-2", 5}, {"science-pack-3", 5}},
      time = 75
    },
    order = "g-e-b"
  },
  {
    type = "technology",
    name = "energy-shield-mk7-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/energy-shield-mk7-equipment.png",
    prerequisites = {"energy-shield-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "energy-shield-mk7-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 5}, {"science-pack-2", 5}, {"science-pack-3", 5}},
      time = 100
    },
    order = "g-e-b"
  },



  {
    type = "technology",
    name = "fusion-reactor-mk2-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk2-equipment.png",
    prerequisites = {"fusion-reactor-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk2-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 50
    },
    order = "g-l"
  },
  {
    type = "technology",
    name = "fusion-reactor-mk3-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk3-equipment.png",
    prerequisites = {"fusion-reactor-mk2-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk3-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 3}, {"science-pack-2", 3}, {"science-pack-3", 3}},
      time = 60
    },
    order = "g-l"
  },
  {
    type = "technology",
    name = "fusion-reactor-mk4-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk4-equipment.png",
    prerequisites = {"fusion-reactor-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk4-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}},
      time = 70
    },
    order = "g-l"
  },
  {
    type = "technology",
    name = "fusion-reactor-mk5-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk5-equipment.png",
    prerequisites = {"fusion-reactor-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk5-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 5}, {"science-pack-2", 5}, {"science-pack-3", 5}},
      time = 80
    },
    order = "g-l"
  },
  {
    type = "technology",
    name = "fusion-reactor-mk6-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk6-equipment.png",
    prerequisites = {"fusion-reactor-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk6-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 6}, {"science-pack-2", 6}, {"science-pack-3", 6}},
      time = 100
    },
    order = "g-l"
  },
  {
    type = "technology",
    name = "fusion-reactor-mk7-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/fusion-reactor-mk7-equipment.png",
    prerequisites = {"fusion-reactor-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "fusion-reactor-mk7-equipment"
      }
    },
    unit =
    {
      count = 200,
      ingredients = {{"science-pack-1", 7}, {"science-pack-2", 7}, {"science-pack-3", 7}},
      time = 120
    },
    order = "g-l"
  }, 



  {
    type = "technology",
    name = "laser-defense-mk2-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk2-equipment.png",
    prerequisites = {"basic-laser-defense-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk2-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 40
    },
    order = "g-m"
  },
  {
    type = "technology",
    name = "laser-defense-mk3-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk3-equipment.png",
    prerequisites = {"laser-defense-mk2-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk3-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 3}, {"science-pack-2", 3}, {"science-pack-3", 3}},
      time = 55
    },
    order = "g-m"
  },
  {
    type = "technology",
    name = "laser-defense-mk4-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk4-equipment.png",
    prerequisites = {"laser-defense-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk4-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}},
      time = 65
    },
    order = "g-m"
  },
  {
    type = "technology",
    name = "laser-defense-mk5-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk5-equipment.png",
    prerequisites = {"laser-defense-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk5-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 5}, {"science-pack-2", 5}, {"science-pack-3", 5}},
      time = 80
    },
    order = "g-m"
  },
  {
    type = "technology",
    name = "laser-defense-mk6-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk6-equipment.png",
    prerequisites = {"laser-defense-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk6-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 6}, {"science-pack-2", 6}, {"science-pack-3", 6}},
      time = 100
    },
    order = "g-m"
  },
  {
    type = "technology",
    name = "laser-defense-mk7-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/laser-defense-mk7-equipment.png",
    prerequisites = {"laser-defense-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "laser-defense-mk7-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 7}, {"science-pack-2", 7}, {"science-pack-3", 7}},
      time = 120
    },
    order = "g-m"
  },



  {
    type = "technology",
    name = "battery-mk3-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk3-equipment.png",
    prerequisites = {"battery-mk2-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk3-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk4-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk4-equipment.png",
    prerequisites = {"battery-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk4-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 3}, {"science-pack-2", 3}, {"science-pack-3", 3}},
      time = 40
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk5-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk5-equipment.png",
    prerequisites = {"battery-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk5-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}},
      time = 45
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk6-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk6-equipment.png",
    prerequisites = {"battery-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk6-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 5}, {"science-pack-2", 5}, {"science-pack-3", 5}},
      time = 50
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk7-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk7-equipment.png",
    prerequisites = {"battery-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk7-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 6}, {"science-pack-2", 6}, {"science-pack-3", 6}},
      time = 55
    },
    order = "g-i-b"
  },





  {
    type = "technology",
    name = "exoskeleton-mk2-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk2-equipment.png",
    prerequisites = {"basic-exoskeleton-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk2-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 1}, {"science-pack-2", 1}, {"science-pack-3", 1}},
      time = 30
    },
    order = "g-h"
  },
  {
    type = "technology",
    name = "exoskeleton-mk3-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk3-equipment.png",
    prerequisites = {"exoskeleton-mk2-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk3-equipment"
      }
    },
    unit =
    {
      count = 50,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-h"
  },
  {
    type = "technology",
    name = "exoskeleton-mk4-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk4-equipment.png",
    prerequisites = {"exoskeleton-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk4-equipment"
      }
    },
    unit =
    {
      count = 75,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 40
    },
    order = "g-h"
  },
  {
    type = "technology",
    name = "exoskeleton-mk5-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk5-equipment.png",
    prerequisites = {"exoskeleton-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk5-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 45
    },
    order = "g-h"
  },
  {
    type = "technology",
    name = "exoskeleton-mk6-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk6-equipment.png",
    prerequisites = {"exoskeleton-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk6-equipment"
      }
    },
    unit =
    {
      count = 130,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 39
    },
    order = "g-h"
  },
  {
    type = "technology",
    name = "exoskeleton-mk7-equipment",
    icon = "__AdvancedEquipment__/graphics/technology/exoskeleton-mk7-equipment.png",
    prerequisites = {"exoskeleton-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "exoskeleton-mk7-equipment"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}},
      time = 55
    },
    order = "g-h"
  },





  {
    type = "technology",
    name = "battery-mk3-equipment-horizontal",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk3-equipment-horizontal.png",
    prerequisites = {"battery-mk3-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk3-equipment-horizontal"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk4-equipment-horizontal",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk4-equipment-horizontal.png",
    prerequisites = {"battery-mk4-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk4-equipment-horizontal"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk5-equipment-horizontal",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk5-equipment-horizontal.png",
    prerequisites = {"battery-mk5-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk5-equipment-horizontal"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk6-equipment-horizontal",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk6-equipment-horizontal.png",
    prerequisites = {"battery-mk6-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk6-equipment-horizontal"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },
  {
    type = "technology",
    name = "battery-mk7-equipment-horizontal",
    icon = "__AdvancedEquipment__/graphics/technology/battery-mk7-equipment-horizontal.png",
    prerequisites = {"battery-mk7-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "battery-mk7-equipment-horizontal"
      }
    },
    unit =
    {
      count = 100,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 35
    },
    order = "g-i-b"
  },




  {
    type = "technology",
    name = "advanced-body-armor-mk1",
    icon = "__AdvancedEquipment__/graphics/technology/advanced-body-armor-mk1.png",
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "advanced-body-armor-mk1"
      }
    },
    prerequisites = {"power-armor-2"},
    unit =
    {
      count = 150,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}, {"alien-science-pack", 5}},
      time = 30
    },
    order = "g-c-b"
  },
  {
    type = "technology",
    name = "advanced-body-armor-mk2",
    icon = "__AdvancedEquipment__/graphics/technology/advanced-body-armor-mk2.png",
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "advanced-body-armor-mk2"
      }
    },
    prerequisites = {"advanced-body-armor-mk1"},
    unit =
    {
      count = 150,
      ingredients = {{"science-pack-1", 4}, {"science-pack-2", 4}, {"science-pack-3", 4}, {"alien-science-pack", 10}},
      time = 30
    },
    order = "g-c-b"
  },
  
  
  
  
    {
    type = "technology",
    name = "military-2b",
    icon = "__AdvancedEquipment__/graphics/technology/military2b.png",
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "mini-gun-I"
      },
      {
        type = "unlock-recipe",
        recipe = "minigun-bullet-magazine"
      }
    },
    prerequisites = {"military-2"},
    unit =
    {
      count = 20,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1}
      },
      time = 15
    },
    order = "e-a-b"
  },

 {
    type = "technology",
    name = "military-3b",
    icon = "__AdvancedEquipment__/graphics/technology/military3b.png",
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "mini-gun-II"
      },
      {
        type = "unlock-recipe",
        recipe = "minigun-piercing-bullet-magazine"
      }
    },
    prerequisites = {"military-3"},
    unit =
    {
      count = 50,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 2},
        {"science-pack-3", 1}
      },
      time = 30
    },
    order = "e-a-c"
  },
  
   {
    type = "technology",
    name = "military-4b",
    icon = "__AdvancedEquipment__/graphics/technology/military4b.png",
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "minigun-uranium-bullet-magazine"
      }
    },
    prerequisites = {"military-4"},
    unit =
    {
      count = 50,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 2},
        {"science-pack-3", 1}
      },
      time = 30
    },
    order = "e-a-c"
  },
  

  
   {
    type = "technology",
    name = "military-finall",
    icon = "__AdvancedEquipment__/graphics/technology/military4b.png",
    effects =
    {
     --[[ 
      {
        type = "unlock-recipe",
        recipe = "railgun"
      },
      {
        type = "unlock-recipe",
        recipe = "railgun-dart"
      }
	--]]
	  {
        type = "unlock-recipe",
        recipe = "laser-mini-gun-I"
      },
	  {
        type = "unlock-recipe",
        recipe = "li-ion-battery"
      },	 
	 
    },
    prerequisites = {"military-4b"},
    unit =
    {
      count = 50,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 2},
        {"science-pack-3", 1}
      },
      time = 30
    },
    order = "e-a-c"
  },
  
  
  
  
  
  
  
  
     {
    type = "technology",
    name = "minigun-damage-1",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"military"},
    unit =
    {
      count = 50,
      ingredients =
      {
        {"science-pack-1", 1}
      },
      time = 30
    },
    upgrade = true,
    order = "e-l-a"
  },
  {
    type = "technology",
    name = "minigun-damage-2",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-damage-1"},
    unit =
    {
      count = 100,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1}
      },
      time = 30
    },
    upgrade = true,
    order = "e-l-b"
  },
  {
    type = "technology",
    name = "minigun-damage-3",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.2"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.2"
      }
    },
    prerequisites = {"minigun-damage-2"},
    unit =
    {
      count = 200,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-c"
  },
  {
    type = "technology",
    name = "minigun-damage-4",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.2"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.2"
      }
    },
    prerequisites = {"minigun-damage-3"},
    unit =
    {
      count = 100,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-d"
  },
  {
    type = "technology",
    name = "minigun-damage-5",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.2"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.2"
      }
    },
    prerequisites = {"minigun-damage-4"},
    unit =
    {
      count = 200,
      ingredients =
      {
        {"alien-science-pack", 1},
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-e"
  },
  {
    type = "technology",
    name = "minigun-damage-6",
    icon = "__base__/graphics/technology/bullet-damage.png",
    effects =
    {
      {
        type = "ammo-damage",
        ammo_category = "minigun-bullet",
        modifier = "0.2"
      },
	 {
        type = "ammo-damage",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.2"
      }
    },
    prerequisites = {"minigun-damage-5"},
    unit =
    {
      count = 300,
      ingredients =
      {
        {"alien-science-pack", 1},
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-f"
  },
  {
    type = "technology",
    name = "minigun-speed-1",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.2"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"military"},
    unit =
    {
      count = 50,
      ingredients =
      {
        {"science-pack-1", 1}
      },
      time = 30
    },
    upgrade = true,
    order = "e-l-g"
  },
  {
    type = "technology",
    name = "minigun-speed-2",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-speed-1"},
    unit =
    {
      count = 100,
      ingredients =
      {
        {"science-pack-1", 1}
      },
      time = 30
    },
    upgrade = true,
    order = "e-l-h"
  },
  {
    type = "technology",
    name = "minigun-speed-3",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-speed-2"},
    unit =
    {
      count = 100,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-i"
  },
  {
    type = "technology",
    name = "minigun-speed-4",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-speed-3"},
    unit =
    {
      count = 200,
      ingredients =
      {
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-j"
  },
  {
    type = "technology",
    name = "minigun-speed-5",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-speed-4"},
    unit =
    {
      count = 200,
      ingredients =
      {
        {"alien-science-pack", 1},
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-k"
  },
  {
    type = "technology",
    name = "minigun-speed-6",
    icon = "__base__/graphics/technology/bullet-speed.png",
    effects =
    {
      {
        type = "gun-speed",
        ammo_category = "minigun-bullet",
        modifier = "0.1"
      },
	 {
        type = "gun-speed",
        ammo_category = "minigun-energy-bullet",
        modifier = "0.1"
      }
    },
    prerequisites = {"minigun-speed-5"},
    unit =
    {
      count = 300,
      ingredients =
      {
        {"alien-science-pack", 1},
        {"science-pack-1", 1},
        {"science-pack-2", 1},
        {"science-pack-3", 1}
      },
      time = 60
    },
    upgrade = true,
    order = "e-l-l"
  },
  
    {
    type = "technology",
    name = "solar-panel-equipment-MKII",
    icon = "__AdvancedEquipment__/graphics/technology/solar-panel-equipment-MKII.png",
    prerequisites = {"solar-panel-equipment"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "solar-panel-equipment-MKII"
      }
    },
    unit =
    {
      count = 50,
      ingredients = {{"science-pack-1", 1}, {"science-pack-2", 1}, {"science-pack-3", 1}},
      time = 15
    },
    order = "g-l"
  },
  
      {
    type = "technology",
    name = "solar-panel-equipment-MKIII",
    icon = "__AdvancedEquipment__/graphics/technology/solar-panel-equipment-MKII.png",
    prerequisites = {"solar-panel-equipment-MKII"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "solar-panel-equipment-MKIII"
      }
    },
    unit =
    {
      count = 50,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 15
    },
    order = "g-m"
  },
  
  
        {
    type = "technology",
    name = "advanced-repair-pack-MKII",
    icon = "__AdvancedEquipment__/graphics/technology/repair-pack-MKII.png",
    prerequisites = {"steel-processing" , "military"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "repair-pack-MKII"
      }
    },
    unit =
    {
      count = 50,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}},
      time = 15
    },
    order = "g-m"
  },
  
          {
    type = "technology",
    name = "advanced-repair-pack-MKIII",
    icon = "__AdvancedEquipment__/graphics/technology/repair-pack-MKIII.png",
    prerequisites = {"advanced-repair-pack-MKII"},
    effects =
    {
      {
        type = "unlock-recipe",
        recipe = "repair-pack-MKIII"
      }
    },
    unit =
    {
      count = 50,
      ingredients = {{"science-pack-1", 2}, {"science-pack-2", 2}, {"science-pack-3", 2}},
      time = 15
    },
    order = "g-m"
  },
  
  
  
}
)
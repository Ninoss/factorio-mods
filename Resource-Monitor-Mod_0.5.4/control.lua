		...
		if global.resourceMonitor[player.index] == nil then
			...
			if global.resourceMonitor[player.index].flags == nil then
				...
				if global.resourceMonitor[player.index].flags.MonitorVisible == nil then
					global.resourceMonitor[player.index].flags.MonitorVisible = 1	-- 0 = not visible
																											-- 1 = GUI visible
																											-- 2 = GUI minimized
				end
...
game.on_event(defines.events.on_tick, function(event)
	for _,player in ipairs(game.players) do
		if global.resourceMonitor[player.index] ~= nil and game.tick % global.resourceMonitor[player.index].flags.updateFreq == 0 then

...
script.on_configuration_changed(function() OnConfigurationChanged(event) end)
...
script.on_event(defines.events.on_pre_player_mined_item, function(event) OnPrePlayerMinedItem(event) end)
...
function OnConfigurationChanged(_Event)
    for key,_Splitter in pairs(global.SingleSplitter) do 
        if _Splitter.surface == nil then
            _Splitter.surface = game.surfaces[1]
        end 
    end
end
...
function AddSingleSplitter(_Splitter)
    ...
    local set = {pos = _Position, dir = _Direction, surface = _Surface, lastInSide = 1, lastOut = {1,2} }		
    AddSet(set)	
end

function RemoveSingleSplitter(_Splitter)		
    for key,splitterInDB in pairs(global.SingleSplitter) do 
        if splitterInDB.pos.x == _Splitter.position.x and splitterInDB.pos.y == _Splitter.position.y and _Splitter.surface == splitterInDB.surface then
